...
void rec() {
    // all depends on what first player plays
    // if first player plays trump card => it can only be defeated by higher trump card

    // if first player plays non trump card => it can be defeated by higher non trump card or trump card
}
        ...
        if (color != trump) {
            ...
            while (sz(temp) > 0) {
                if (sz(temp) == 1) {
                    // boroow from trump
                    if (mp[trump].size() > 0) {
                        int bor = *mp[trump].begin();
                        mp[trump].erase(mp[trump].begin());
                        string s2 = to_string(bor);
                        s2.push_back(trump);
                        string s1 = to_string(*temp.begin());
                        s1.push_back(color);
