...
void solve() {
    ...
    if (sz(t)) {
        ...
        // print(temp);
        rep(i, 0, sz(t)) {
            while (sz(temp) > 0 and temp.back() > t[i]) {
                temp.pop_back();
            }
            storeIndex[t[i]].push_back(temp.back());
            temp.pop_back();
        }
        // cout << "after fillling zero elements left" << endl;
        // print(temp);

        // cout << "mapped elements" << endl;
        // for (auto x : storeIndex) {
        //     cout << x.first << "->";
        //     print(x.second);
        // }
        rep(i, 0, n) {
            if (a[i] > 0) {
                b[i] = storeIndex[a[i]].back();
                storeIndex[a[i]].pop_back();
            }
        }
    }
    // cout << "A finised" << endl;
    ...
    if (sz(t)) {
        ...
        rep(i, 0, n) {
            if (a[i] == 0) {
                a[i] = storeIndex[b[i]].back();
                storeIndex[b[i]].pop_back();
            }
        }
    }
    // cout << "after Finishing B" << endl;
    rep(i, 0, n) {
        if (a[i] == 0 or b[i] == 0 or (max(a[i], b[i]) != v[i]) or a[i] > n or b[i] > n or a[i] < 1 or b[i] < 1) {
            cout << "NO" << endl;
            return;
        }
    }
