...
void solve() {
    ...
    // what is the maximum x such that every element of the array >= x
    int low = 0ll, high = 1e18;
    while (high - low > 1) {  // NLogN
    ...
    // // count #elements which are > low
    // int tot = 0;
    // rep(i, 0, n) {
    //     if (v[i] > low) tot++;
    // }
    // int ans = low + (n - 1) * (low - 1);
    // cout << ans + tot << endl;
}
signed main() {
    fastio();
