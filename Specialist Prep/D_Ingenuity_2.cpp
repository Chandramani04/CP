...
void solve() {
    ...
    // odd -> 1st player moves , else 2nd player moves
    string ans = "";
    for (int i = 0; i < n; i++) {
        if (S[i] == 'N') {
            north++;
    ...
    if (count(all(ans), 'H') == 0) {
        int cntN = count(all(S), 'N');
        int cntS = count(all(S), 'S');
        int cntE = count(all(S), 'E');
        int cntW = count(all(S), 'W');
        if (cntN > 0 and cntS > 0) {
            auto it = find(all(S), 'N');
            int idx = it - S.begin();
            ans[idx] = 'H';
            it = find(all(S), 'S');
            idx = it - S.begin();
            ans[idx] = 'H';
        } else if (cntE > 0 and cntW > 0) {
            auto it = find(all(S), 'E');
            int idx = it - S.begin();
            ans[idx] = 'H';
            it = find(all(S), 'W');
            idx = it - S.begin();
