...
void solve1() {
    ...
    rep(i, 0, n) {
        ...
        // present in the map , and have index >=i
        int oneXor = (v[i] ^ 1);
        if (!(sz(mp[oneXor]) and mp[oneXor].lower_bound(i) != mp[oneXor].end())) {
            oneXor = 2e9;
        }
        int twoXor = (v[i] ^ 2);
        if (!(sz(mp[twoXor]) and mp[twoXor].lower_bound(i) != mp[twoXor].end())) {
            twoXor = 2e9;
        }
        int threeXor = (v[i] ^ 3);
        if (!(sz(mp[threeXor]) and mp[threeXor].lower_bound(i) != mp[threeXor].end())) {
            threeXor = 2e9;
        }
        int bestEle = min({zeroXor, oneXor, twoXor, threeXor});
        auto it = mp[bestEle].lower_bound(i);
        int pos2 = *it;                             // position after swapping
        mp[v[i]].erase(mp[v[i]].find(i));           // deleting current index from set of v[i]
        mp[v[i]].insert(pos2);                      // inserting new position of v[i] in set of v[i]
        mp[bestEle].erase(mp[bestEle].find(pos2));  // deleting position taken by v[i] from set of bestEle
        mp[bestEle].insert(i);                      // adding new position of bestEle in it's set
        swap(v[i], v[pos2]);                        // swapping
    }
    print(v);
}
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            int nno = no ^ j;
            if (mp.find(nno) != mp.end() && mp[nno].lower_bound(i) != mp[nno].end()) {
                mn = min(mn, nno);
            }
        }
        int nno = mn;
        // cout << v[i] << " " << nno << endl;
        if (mp.find(nno) != mp.end() && nno < no) {
            int pos1 = i;
            auto it = mp[nno].lower_bound(i);

            int pos2 = *it;
            // if (mp[no].find(i) != mp[no].end())
            mp[no].erase(mp[no].find(i));
            mp[no].insert(pos2);
            // if (mp[nno].find(i) != mp[nno].end())
            mp[nno].erase(mp[nno].find(pos2));
            mp[nno].insert(pos1);
            swap(v[i], v[pos2]);
        }
    }
