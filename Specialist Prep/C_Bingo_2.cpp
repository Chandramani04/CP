...
void solve() {
    ...
    rep(i, 0, n) {
        vi temp = grid[i];
        row.push_back(set<int>(all(temp)));
    }
    rep(j, 0, n) {
        vi temp;
        rep(i, 0, n) {
        ...
        col.push_back(set<int>(all(temp)));
    }
    set<int> diag1, diag2;
    rep(i, 0, n) {
        rep(j, 0, n) {
            if (i == j) {
                diag1.insert(grid[i][j]);
            }
        }
    }
    rep(i, 0, n) {
    ...
    if (checkDiag(diag1)) {
        ans = min(ans, whichTurn(diag1));
    }
    if (checkDiag(diag2)) {
        ans = min(ans, whichTurn(diag2));
    }
    if (ans == 1e9) {
        cout << -1 << endl;
        return;
