...
void generateSubset(int i, vector<int>& coin, vi carry) {
    if (i == sz(coin)) {
        comb.pb(carry);
        return;
    }
    // don't pick
    generateSubset(i + 1, coin, carry);
    // pick
    carry.pb(coin[i]);
    generateSubset(i + 1, coin, carry);
    carry.pop_back();
...
void solve() {
    int n, m, k;
    cin >> n >> m >> k;
    vector<pair<vector<int>, char>> store;
    rep(j, 0, m) {
        int t;
        cin >> t;
        input(cur, t);
    ...
    vi carry;
    generateSubset(0, coins, carry);
    int ans = 0;
    for (auto realKeys : comb) {
        set<int> keys(all(realKeys));
        bool ok = true;
        for (auto [cur, res] : store) {
            int isReal = 0;
            for (auto x : cur) {
                if (keys.count(x)) {
                    isReal++;
