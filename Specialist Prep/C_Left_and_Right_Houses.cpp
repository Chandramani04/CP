...
int getLengthOfOptimalCompression(string s, int k) {
    string c = compressedString(s);
    int n = c.size();
    string noSolo = "";
    string solo = "";
    for (int i = 0; i < sz(c) - 1; i++) {
        if ((c[i] >= 'a' and c[i] <= 'z') and (c[i + 1] >= '1' and c[i + 1] <= '9')) {
            noSolo.push_back(c[i]);
            for (int j = i + 1; j < n; j++) {
                if (!(c[j] >= '0' and c[j] <= '9')) {
                    i = j - 1;
                    break;
                }
                noSolo.push_back(c[j]);
            }
        } else {
            solo.push_back(c[i]);
        }
    }
    if (c.back() >= 'a' and c.back() <= 'z') solo.push_back(c.back());
    cout << c << endl;
    cout << "solo = " << solo << endl;
    cout << "noSolo = " << noSolo << endl;

    vector<int> v;
    for (int i = 0; i < noSolo.size(); i += 2) {
        v.push_back((noSolo[i + 1]) - 48);
    }
...
void solve() {
    ...
    int k = 0;
    cin >> k;
    cout << getLengthOfOptimalCompression(word, k);
