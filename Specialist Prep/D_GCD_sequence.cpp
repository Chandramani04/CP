...
void solve() {
    ...
    vi curGcd(n - 1);
    rep(i, 0, n - 1) {
        curGcd[i] = gcd(v[i], v[i + 1]);
    }
    if (is_sorted(all(curGcd))) {
        cout << "YES" << endl;  // we removes last element
        return;
    }
    auto fx = [&](int remove) {
        vi temp;
        ...
        vi tempGcd(n - 2);
        rep(i, 0, n - 2) {
            tempGcd[i] = gcd(temp[i], temp[i + 1]);
        }
        return is_sorted(all(tempGcd));
    };
    // print(curGcd);
    rep(i, 1, sz(curGcd)) {
        if (curGcd[i] < curGcd[i - 1]) {
            if (fx(i - 1)) {
                cout << "YES" << endl;
                return;
            }
