...
void solve() {
    ...
    if (k == 0) {
        ...
        int blockSize = pow(3, k - 1);
        int centerBlockSizeLeft = pow(3, k - 2);
        int centerBlockSizeRight = 2 * centerBlockSizeLeft;
        vector<string> otherBlock, centerBlock;
        for (int i = 0; i < blockSize; i++) {
            string s = "";
            for (int j = 0; j < blockSize; j++) {
                if (i >= centerBlockSizeLeft and i < centerBlockSizeRight and j >= centerBlockSizeLeft and j < centerBlockSizeRight) {
                    s.push_back('.');
                    // cout << ".";
                } else {
                    s.push_back('#');
        ...
        for (int i = 0; i < blockSize; i++) {
            ...
            centerBlock.push_back(s);
        }
        vector<string> other, cent;
        for (auto x : otherBlock) {
            // cout << x << endl;
        }
        rep(i, 0, blockSize) {
            string s = "";
