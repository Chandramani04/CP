...
int maxIndex(int n, int F) {
    // Calculate the sum of the first n natural numbers
    int S_n = n * (n + 1) / 2;

    // If F is out of the range [0, S_n], we don't need to worry about it
    if (F > S_n) {
        return S_n;
    }

    // Try to avoid the forbidden index F
    int currentSum = S_n;
    for (int k = n; k >= 1; --k) {
        int tempSum = currentSum - k;  // Imagine skipping step k
        if (tempSum == F) {
            continue;  // Skip this step and see if it avoids F
        }
        if (tempSum < F) {
            // The largest possible sum without hitting F
            return currentSum - k;
        }
    }

    // If we can't avoid F by skipping any single step, return S_n - 1
    return S_n - 1;
}
...
void solve() {
    cout<<partitionArray(4,{3,3,5,2})<<endl;
}
signed main() {
    fastio();
...
*/
/* LIS in NLogN
vector<int>print_lis(vector<int>&numbers){
int n = v.size();
vector<int>temp;
temp.push_back(v[0]);
vector<int>lis;
