...
void solve() {
    ...
    input(v, n);
    int ans = 0;
    map<tuple<int, int, int>, int> general;
    map<pii, int> firstTwo, lastTwo, firstLast;
    for (int i = 0; i < n - 2; i++) {
        // first two matches, last doesn't
        // ans+=  #pairs of first two -  #tuples of a[i],a[i+1],a[i+2] => this way all the pairs with distinct last element
        // will be counted
        ans += firstTwo[{v[i], v[i + 1]}] - general[{v[i], v[i + 1], v[i + 2]}];
        firstTwo[{v[i], v[i + 1]}]++;
        // last two matches , first doesn't
        // ans+=  #pairs of last two -  #tuples of a[i],a[i+1],a[i+2]
        ans += lastTwo[{v[i + 1], v[i + 2]}] - general[{v[i], v[i + 1], v[i + 2]}];
        lastTwo[{v[i + 1], v[i + 2]}]++;
        // firstlast matches , middle doesn't
        // ans+=  #pairs of first and last  -  #tuples of a[i],a[i+1],a[i+2]
        ans += firstLast[{v[i], v[i + 2]}] - general[{v[i], v[i + 1], v[i + 2]}];
        firstLast[{v[i], v[i + 2]}]++;
        general[{v[i], v[i + 1], v[i + 2]}]++;
    }
    cout << ans << endl;
}
signed main() {
