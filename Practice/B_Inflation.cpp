...
/*---------------------------Code Begins---------------------------------------*/
int check(int inf, int start, int k) {
    int l = 0, r = 1e9;
    double cmd = (inf * 100.0) / start;
    if (cmd <= k) return 0;
    while (r - l > 1) {
        int mid = (l + r) / 2;
        double temp = (inf * 100.0) / (start + mid);
        if (temp <= k) {
            r = mid;
        } else {
            l = mid + 1;
...
int check2(int inf, int start, int k) {
    int l = 0, r = 1e15;
    double a = inf * 100.0;
    double b = start * k;
    if (a <= b) {
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        cin >> arr[i];
        int cur_inf = arr[i];
        int cur_start = pref[i - 1];
        if (i > 1) {
            int extra = check2(cur_inf, cur_start, k);
            ans += extra;
            pref[i] = pref[i - 1] + extra + arr[i];
        } else {
            pref[i] = arr[i];
        }
    }
