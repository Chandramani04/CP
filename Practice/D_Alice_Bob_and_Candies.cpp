...
void solve() {
    ...
    while (true) {
        if (l > r) break;
        if (move == 0) {
            // l points to an element which is not picked yet
            int temp = 0;
            while (l <= r and temp <= prev) {
                temp += v[l];
                l++;
            }
            alice += temp;
            ...
            int temp = 0;
            while (r >= l and temp <= prev) {
                temp += v[r];
                r--;
            }
            // r points to an element which is not picked yet
            bob += temp;
            prev = temp;
            move = 0;
            cnt_moves++;
