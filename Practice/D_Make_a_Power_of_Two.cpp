...
int rec(int n) {
    ...
    reverse(s);
    // pop last element and add any element from 0 to 9
    // to make it power of 2
    int ans = 1e9;
    if (s.back() == '0') {
        s.pop_back();
        int ans = 1;
...
void cmd() {
    for (int i = 0; i <= 63; i++) {
        ...
        //     break;
        // } else
        power_of_two.insert(to_string(temp));
    }
...
void solve() {
    ...
    for (auto z : power_of_two) {
        int i = 0, j = 0, del = 0;
        while (j < sz(z) and i < sz(str)) {
            if (str[i] == z[j]) {
                i++;
                j++;
            } else {
