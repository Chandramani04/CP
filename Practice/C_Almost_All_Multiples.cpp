...
int spf[MAXN];
map<int, set<int>> gm;
vector<int> sieve(int n, int x) {
    int* arr = new int[n + 1]();
    vector<int> vect;
    for (int i = 2; i <= n; i++)
        if (arr[i] == 0) {
            vect.push_back(i);
            if (i != x) gm[i].insert(i);
            for (int j = 2 * i; j <= n; j += i) {
                if (spf[j] == i and j != x) gm[i].insert(j);
                arr[j] = 1;
            }
        }
    return vect;
...
void solve() {
    ...
    // /*
    //     i ke spf pe jao , usme se ek element uthao jo >=i ho and ele%i = 0 ho
    // */
    // vi ans(n + 1, 0);
    // ans[n] = 1;
    // ans[1] = x;
