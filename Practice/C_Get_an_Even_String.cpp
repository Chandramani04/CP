...
int rec(int i, string& s) {
    int n = sz(s);
    if (i == n) return 0;
    if (i == n - 1) return 1;  // we are at last index , so we can't pair it up with any other element
    ...
    if (j == n) {
        // no other matching , we have to remove it for sure
        return dp[i] = 1 + rec(i + 1, s);
    } else {
        int gap = j - i - 1;
        int remove_current = 1 + rec(i + 1, s);
        int remove_gap_elements = gap + rec(j + 1, s);
        return dp[i] = min(remove_current, remove_gap_elements);
    }
}
