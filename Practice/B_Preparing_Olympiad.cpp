...
/*---------------------------Code Begins---------------------------------------*/
int rec(int i, vi carry, vi& test, int l, int r, int x) {
    int n = test.size();
    if (i == n) {
        if (sz(carry) < 2) return 0;
        int tsum = sum(carry);
        if (tsum >= l and tsum <= r and maxel(carry) - minel(carry) >= x) return 1;
        return 0;
    }
    // pick the ith element
    carry.push_back(test[i]);
    int ans1 = rec(i + 1, carry, test, l, r, x);
    carry.pop_back();
    // leave the ith element
    int ans2 = rec(i + 1, carry, test, l, r, x);
    return ans1 + ans2;
}
void solve() {
    int n, l, r, x;
    ...
    vi carry;
    input(test, n);
    cout << rec(0, carry, test, l, r, x) << endl;
}
signed main() {
    fastio();
