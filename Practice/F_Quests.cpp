...
void solve() {
    ...
    sorted(rewards);
    reverse(rewards);
    // if we can get amount c in days days without using any coin more than once => answer is Infinity
    int useSingleTime = 0;
    rep(i, 0, min(n, days)) {
        useSingleTime += rewards[i];
    }
    if (useSingleTime >= c) {
        cout << "Infinity" << endl;
        return;
    }
    // when k = 0 , we can use maximum coin everytime 
    if (maxel(rewards) * days < c) {
        cout << "Impossible" << endl;
        return;
    }
    
    auto canMakeCbyUsingK = [&](int k) {
        if (k == 0) {
            return (maxel(rewards) * days >= c);
        }

        /*
            say d = 10 , k = 3 , we can follow below pattern to use quests , sorted in descending order
            a1 a2 a3 a4 a1 a2 a3 a4 a1 a2
            // maximum distinct values we need = k + 1 , because after k day , we can use the same quest again
        */
        int cnt = 0;
        int cmp = days / (k + 1);  // number of times we can use all k + 1 quests
        int rem = days % (k + 1); // #days we will use the largest ith quest 
