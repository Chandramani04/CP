...
void solve() {
    ...
    // print(a);
    // print(b);
    // if(binary_search(all(b),1)){
    //     cout<<"present in b"<<endl;
    // }
    // cout << "working" << endl;
    // return;

    auto ok = [&](int target) {
        if (binary_search(all(b), target)) return true;
        else return false;
    };
    auto ok2 = [&](int target) {
        if (binary_search(all(a), target)) return true;
        else return false;
    };
