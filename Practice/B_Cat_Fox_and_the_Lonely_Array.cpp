...
void solve() {
    ...
    /*
        Concept =>
        Check how many bits available in the each subarray of size k 
        if for each subarray atleast one bit if set , then the OR of that subarray will be added by 1<<i
    */

    // if same element => ans 1 
    set<int>checkSingle(all(v));
    if(sz(checkSingle)==1){
        cout<<1<<endl;
        return;
    }
    ...
    auto check = [&](int k) {
        // check is or of every subarray is same or not
