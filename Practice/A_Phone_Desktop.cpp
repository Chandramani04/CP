...
void solve() {
    int x, y;
    cin >> x >> y;
    // ek screen se 2 square maximum bana skte hain
    // after making 2 square ek single screen me 7 solo cell bachte hain
    // if we make 1 square 11 solo cell bachte hain hain
    int ans = 0;
    if (y & 1) {
        int comsq = y / 2;
        ans += comsq;
        ans++;
        int solocell = comsq * 7;
        solocell += 11;
        x -= solocell;
        if (x > 0) {
            ans += ((x + 14) / 15);
