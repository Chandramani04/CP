...
void solve() {
    ...
    input(v, n);
    int cur_mex = MEX(v);
    int new_mex = cur_mex + 1;
    // if new mex is not present we just have to insert a cur_mex at any position
    if (count(all(v), new_mex) == 0) {
        if (cur_mex == n) {
            cout << "NO" << endl;
            return;
        }
    ...
    // if new_mex is present in the array
    else {
        // a b c d CM CM NM NM e f
        // option 1
        // replace all the eleemnts bewteen 1st next_mex and last next_mex with cur_mex
        int left = -1, right = -1;
        rep(i, 0, n) {
            if (v[i] == new_mex) {
                left = i;
