...
// 10^3  5e3 constraints

// dp[i][head/tail][counter]
int iterative() {
    int dp[n + 1][2][n + 1];
    ...
    // base case
    // if head comes
    dp[0][0][1] = x[0] + bonus[1];
    // if tail comes
    dp[0][1][0] = x[0];

    rep(i, 1, n) {
        int ctpt = 0;  // current tail previous tail
        ctpt = dp[i - 1][1][0] + x[i] + bonus[0 + 1];

        int ctph = 0;  // current tail previous head
        for (int counter = 1; counter < i; counter++) {
            ctph = max(ctph, dp[i - 1][0][counter] + x[i]);
        }

        for (int headcounter = 1; headcounter <= i; headcounter++) {
            int head = dp[i - 1][0][headcounter - 1] + x[i] + bonus[headcounter];
            // if preivous was head
            int tail = dp[i - 1][1][0] + x[i] + bonus[0 + 1];

            dp[i][0][headcounter] = max(head, tail);
        }
        dp[i][1][0] = max(ctpt, ctph);
    }
    int ans = 0;
    for (int counter = 0; counter <= n; counter++) {
        int head = dp[n - 1][0][counter];
        int tail = dp[n - 1][1][0];
        ans = max(ans, max(head, tail));
    }
    return ans;
}

int f(int i, int money, int counter) {
    if (i == n) {
        return money;
    }
    int head = 0;
    int tail = 0;
    // for head
    head = f(i + 1, money + x[i] + bonus[counter + 1], counter + 1);
    tail = f(i + 1, money + x[i], 0);
...
void solve() {
    ...
    // dp[i][counter] => till i , we have counter heads in th sequence
    ...
    for (int i = 1; i < n; i++) {
        for (int counter = 1; counter <= i; counter++) {
            // head is must here
            dp[i][counter] = dp[i - 1][counter - 1] + x[i] + bonus[counter];
        }
        // tail is must here
        dp[i][0] = 0 ;  
        for (int counter = 0; counter < i; counter++) {
            dp[i][0] = max(dp[i][0], dp[i - 1][counter]);
        }
    }
    int ans = 0;
    for (int counter = 0; counter <= n; counter++) {
        ans = max(ans, dp[n - 1][counter]);
    }
    cout << ans << endl;
}
signed main() {
