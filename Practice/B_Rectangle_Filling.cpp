...
void solve() {
    ...
    // if diagonal corners are same

    // if not same , bb ww on same side

    // if top is of same color and bottom row is of same color , but both rows are not same color

    // col left col != col of  right col
    set<char> topRow, bottomRow, leftColumn, rightColumn;
    rep(i, 0, m) topRow.insert(v[0][i]);
    rep(i, 0, m) bottomRow.insert(v[n - 1][i]);

    if (sz(topRow) == 1 and sz(bottomRow) == 1) {
        auto ele1 = *topRow.begin(), ele2 = *bottomRow.begin();
        if (ele1 != ele2) {
            cout << "NO" << endl;
            return;
        }
    ...
    rep(j, 0, n) leftColumn.insert(v[j][0]);
    rep(j, 0, n) rightColumn.insert(v[j][m - 1]);

    if (sz(leftColumn) == 1 and sz(rightColumn) == 1) {
        auto ele1 = *leftColumn.begin(), ele2 = *rightColumn.begin();
        if (ele1 != ele2) {
            cout << "NO" << endl;
            return;
        }
