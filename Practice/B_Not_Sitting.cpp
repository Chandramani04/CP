...
void solve() {
    int n, m;
    cin >> n >> m;
    // for each cell of grid[n][m] find the max distance from all 4  corner of the grid from that cell
    vi maxdis;
    repo(i, 1, n) {
        repo(j, 1, m) {
            int left_corner = abs(i - 1) + (abs(j - 1));  // (i,j)->(1,1)
            int right_corner = abs(i - 1) + abs(j - m);   // (i,j)->(1,m)
            int bottom_corner = abs(i - n) + abs(j - 1);  // (i,j)->(n,1)
            int top_corner = abs(i - n) + abs(j - m);     // (i,j)->(n,m)
            maxdis.push_back(max({left_corner, right_corner, bottom_corner, top_corner}));
        }
    }
    sorted(maxdis);
    int total = (n * m) - 1;
