...
void solve() {
    ...
    // N = 2*cnt2 + 5*cnt5 + d (d is the remaining part)
    // cout << cnt2 << " " << cnt5 << endl;
    // min(cnt2,cnt5) zeros are already present
    // rem = max(cnt2,cnt5)-min(cnt2,cnt5)
    // if we can complement of rem , then we can get the rem zeros too
    int extra_needed = 1;
    // if cnt2<cnt5
    while (cnt2 < cnt5 and extra_needed * 2 <= m) {
        extra_needed *= 2;
        cnt2++;
    }
    // else if cnt5<cnt2
    while (cnt5 < cnt2 and extra_needed * 5 <= m) {
        extra_needed *= 5;
        cnt5++;
    }
    // now we have multiply with extraneeded
    // we get max(cnt2,cnt5) zeros
    // now if m is more tha extraneed
    // we need to extract most no. of 10's from m
    while (extra_needed * 10 <= m) {
        extra_needed *= 10;
    }

    if (extra_needed == 1) {
        // we can't get more zeros
        cout << n0 * m << endl;
        return;
    }
    // our maximum zeros value is fixed now
    // we have to increase our answer now
    // k>1
    // what k can be , a multiple of 2,or 5 or 10
    // suppose k = 100 , m = 400
    // we can't  make k = 1000 as it will exceed m
    // but we can make k = 200,300.. till 400
    // so we make k max
    extra_needed *= m / extra_needed;
    // m/ extraneeded can't be zero
    // because we have already handled that case k<=m in every step
    cout << n0 * extra_needed << endl;
}
signed main() {
    fastio();
