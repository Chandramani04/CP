...
void solve() {
    ...
    // make all elements +ve , is a neg is fount , make maxel > abs(minel)

    if (is_sorted(all(v))) {
        cout << 0 << endl;
        return;
    }
    int mx = maxel(v), mxi = maxidx(v);
    int mn = minel(v), mni = minidx(v);
    vpii ans;

    // if mn = -ve , mx <=0
    // make last element lease -ve
    // n-2 -> 0 all elements
    if (mn < 0 and mx <= 0) {
        v[n - 1] += mn;// 1
        v[n - 1] += mn;// 1 
    ...
    if (mn < 0 and mx > 0) {
        // make mx > abs(mn)
        // costs atmax 6  operation 1 2 4 8 16 24
        while (v[mxi] < abs(v[mni])) {
            ans.pb({mxi, mxi});
            v[mxi] *= 2;
        }
        // cout<<v[mxi]<<endl;
        // make every element +ve
        // costs 20 operations
        rep(i, 0, n) {
            ans.pb({i, mxi});
            v[i] += v[mxi];
