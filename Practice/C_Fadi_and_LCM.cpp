...
/*
    ...
    ll ans=0;
    ans=max(ans,min(m,ll(s.size()+one)));
    ans=max(ans,min(m,ll(s.size()+two)));
    vl t;
    for(auto it:s){
        t.pb(it);
    }
    for(int j=0;j<t.size();j++){
        ...
        //less than i
        //more than i
        ll left_side_elements=j;
        ll right_side=t.size()-j-1;
        // debug(left_side_elements)
        // debug(right_side)
        ll i=t[j];
        ll left=min(i-1,left_side_elements+one);
        ll right=min(m-i,right_side+two);
        ans=max(ans,min(m,1+left+right));
    }
    cout<<ans<<nl;
*/
