...
void solve() {
    ...
    input(v, n);
    if (v[0] == n) {
        // we can't put n at first position therefore we have to bring N-1 to first position
        auto it = find(v.begin(), v.end(), n - 1);
        int idx = it - v.begin();

        // if n-1 is located at last position , then we have two options
        // swap n-1 with n-1
        ...
        vi ans;
        for (auto x : sufix) ans.pb(x);
        for (auto x : mid) ans.pb(x);
        for (auto x : prefix) ans.pb(x);
        print(ans);

    } else {
        // n will always be at first position
        if (v[n - 1] == n) {
            int cnt = 0;
            for (int i = n - 2; i > 0; i--) {
                if (v[i] > v[0])
            ...
            // from pos[n] to n-1 we have to use as sufix
