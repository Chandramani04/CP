...
/*ALTERNATIVE SOLUTION */
    int cnt = 0;
    // #of pairs from i + 1 to k , such that no pair should contain either b[i] or g[i]
    // also any further pair can't contain both b[i] and g[i] that is current pair
    for (int i = 0; i < n; i++) {
        boy[ready[i].F]--;
        girl[ready[i].S]--;
        int rem = n - 1 - i;
        int nob = boy[ready[i].F];
        int nog = girl[ready[i].S];
        int temp = rem - nob - nog;
        cnt += max(0ll, temp);
    }
    cout << cnt << endl;
}
