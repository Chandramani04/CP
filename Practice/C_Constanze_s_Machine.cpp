...
int rec(int i) {
    ...
    if (i < n - 1) {
        ...
        if (temp == "uu" or temp == "nn") {
            // it was originally w
            int changed = 1 + rec(i + 2);
            // no it was originally u and next one is also u
            int unchanged = rec(i + 1);
            int f = changed % MOD + unchanged % MOD;
            return cache[i] = f % MOD;
        }
    }
    int f = rec(i + 1);
    return cache[i] = f % MOD;
