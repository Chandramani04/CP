...
void ver() {
    ...
    function<int(int, int)> go = [&](int i, int cur) {
        ...
        if ((cur + a[i]) % 3 == 0) {
            ...
            ans = max(go(i + 1, (cur + a[i]) % 3), go(i + 1, 0));
        }
        return dp[i][cur] = ans;
    };
    cout << go(0, 0);
...
int cache[200005][3];
int f(int i, int prevRem, string& s) {
    int n = sz(s);
    if (i == n) {
        return 0;
    }
    if (cache[i][prevRem] != -1) return cache[i][prevRem];
    int curDigit = s[i] - '0';
    // either include current digit
    int Sum = (prevRem + curDigit) % 3;
    ...
    if (Sum == 0) {
        ...
        // must include
        include = f(i + 1, Sum, s);                // addinng current digits to previous subarray
        exclude = f(i + 1, curDigit % 3, s);       // starting a new subarray from current digit
        int leaveCurrentElement = f(i + 1, 0, s);  // not including in any paritioning
        ans = max({include, exclude, leaveCurrentElement});
    }
    return cache[i][prevRem] = ans;
}
