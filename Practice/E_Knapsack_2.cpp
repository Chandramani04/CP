...
void solve() {
    ...
    // dp[i][j] = minimum weight need to get value j using first i items
    // minimum -> initialize with maximum
    int dp[n][100001];
    for (int i = 0; i < n; i++) {
    ...
    // if we have 0 value , no weight needed
    for (int i = 0; i < n; i++) {
        dp[i][0] = 0;
    }
    // picking 1st element
    ...
    for (int i = 1; i < n; i++) {
        for (int j = 0; j <= 100000; j++) {
            ...
            int take = 1e18;
            if (j >= value[i]) {
                take = weight[i] + dp[i - 1][j - value[i]];
            }
