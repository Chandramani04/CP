...
void solve() {
    ...
    cin >> a >> b;
    int zero = 0, one = 0;
    // a[i] = 0 and a[i+1] = 1 or  a[i] = 1 and a[i+1] = 0
    // b[i] = 1 and b[i+1] = 0 or  b[i] = 0 and b[i+1] = 1
    int usex = 0, usey = 0;
    // consecutive one and zeros
    int cz = 0, co = 0;
    auto usex_condition = [&](int i) {
    ...
    rep(i, 0, n) {
        if (a[i] != b[i]) {
            if (i < n - 1 and usex_condition(i)) {
                usex++;
                i++;
                continue;
            }

            // consecutive 0's or 1's case
            if (i < n - 1) {
                if (a[i] == '0' and a[i + 1] == '0' and b[i + 1] == '1') {
                    cz++;
                    // usey += 2;
                    i++;
                    continue;
                }
                if (a[i] == '1' and a[i + 1] == '1' and b[i + 1] == '0') {
                    co++;
                    // usey += 2;
                    i++;
                    continue;
    ...
    if (!(same_parity(zero, one))) {
        cout << -1 << endl;
        return;
    }

    // how many czeros we can map with ones
    // cout << co << " " << cz << " " << zero << " " << one << endl;
    if ((usex == 1 or cz == 1 or co == 1) and zero == 0 and one == 0) {
        cout << min(x, 2 * y) << endl;
        return;
    }
    cout << (2 * usex + zero + one) / 2 * y << endl;
    return;
    usey = 0;
    if (one > 0) {
        if (2 * cz <= one) {
    ...
    // how many cones we can map with zeros
    if (zero > 0) {
        if (2 * co <= zero) {
            usey += 2 * co;
            zero -= 2 * co;
    ...
    cout << (usex * x) + (usey * y) + (((zero + one) / 2) * y) + (co + cz) * 2 * y << endl;
}
void test() {
    int n, x, y;
    cin >> n >> x >> y;
    ...
    cin >> a >> b;
    vi xorarray(n);
    rep(i, 0, n) xorarray[i] = (a[i] - '0') ^ (b[i] - '0');
    // print(xorarray);
    int ones = count(all(xorarray), 1);
    if (ones & 1) {
        cout << -1 << endl;
        return;
    }
    int usex = 0, usey = 0;
    rep(i, 0, n) {
        if (xorarray[i] == 1 and (i < n - 1 and xorarray[i + 1] == 1)) {
            usex++;
            i++;
        } else if (xorarray[i] == 1) {
            usey++;
        }
    }
    if (ones == 2 and usex == 1) {
        cout << min(x, 2 * y) << endl;
        return;
    }
    cout << ones / 2 * y << endl;
    ...
    if (x < 2 * y) {
        ans += (usex * x) + (usey / 2) * y;
        // cout << (usex * x) + (usey * y) << endl;
    } else {
        int notconsecutives = ones - 2 * usex;
        //  for not consecutive ones we can easily use y among themseleves
        ans += (notconsecutives / 2) * y;
        ans += (usex * 2 * y);
        // cout << (ones / 2) * y << endl;
    }
    cout << ans << endl;

    // cout << min((ones / 2 )* y, min(usex * x, 2 * usex * y) + (usey * y)) << endl;
}
signed main() {
    fastio();
