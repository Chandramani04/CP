...
void solve() {
    ...
    // end goal =  [Capital......Small.........]
    vector<int> dp1(n, 0), dp2(n, 0);
    // dp1 will store the minimum number of operations to convert the string to [Capital......Small.........]
    // for dp1 , until we reached last capital letter, we will convert current letter to capital letter

    dp1[0] = (s[0] >= 'a' and s[0] <= 'z');
    int tc = cntCapital;
    for (int i = 1; i < n; i++) {
        if (tc == 0) break;
        bool ok = false;
    ...
    // dp2 will store the minimum number of operations to convert the string to [Small.........Capital......]
    // for dp2 , until we reached last small letter, we will convert current letter to small letter
    dp2[n - 1] = (s[n - 1] >= 'A' and s[n - 1] <= 'Z');
    int ts = cntSmall;
    for (int i = n - 2; i >= 0; i--) {
        if (ts == 0) break;
        bool ok = false;
