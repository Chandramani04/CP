...
void solve() {
    ...
    if (!(gap & 1)) {
        // either bob will will on draw
        int alicemoves = (gap / 2);
        int alice_left_maxmove = max(1ll, ya - alicemoves);
        int alice_right_maxmove = min(w, ya + alicemoves);
        int bobmoves = (gap / 2);
        int bob_left_maxmove = max(1ll, yb - bobmoves);
        int bob_right_maxmove = min(w, yb + bobmoves);

        // if bob is standing right of alice
        if (yb >= ya) {
            if (bob_left_maxmove <= alice_left_maxmove) {
                cout << "Bob" << endl;
            } else
                cout << "Draw" << endl;
            return;
        }
        if (bob_right_maxmove >= alice_right_maxmove) {
            cout << "Bob" << endl;
        } else
            cout << "Draw" << endl;
        return;
    } else {
        // either alice win or draw
        int alicemoves = ((gap + 1) / 2);
        int bobmoves = gap / 2;
        int alice_left_maxmove = max(1ll, ya - alicemoves);
        int alice_right_maxmove = min(w, ya + alicemoves);
        int bob_left_maxmove = max(1ll, yb - bobmoves);
        int bob_right_maxmove = min(w, yb + bobmoves);
        // cout<<alice_left_maxmove<<" "<<bob_left_maxmove<<endl;
        // cout<<alice_right_maxmove<<" "<<bob_right_maxmove<<endl;
        // if bob is standing right of alice
        if (yb > ya) {
            // bob will try to as much right as possible
            if (bob_right_maxmove <= alice_right_maxmove) {
                cout << "Alice" << endl;
            } else
                cout << "Draw" << endl;
        } else if (yb<ya) {
            // bob will go as much as left possible
            if (bob_left_maxmove >= alice_left_maxmove) {
                cout << "Alice" << endl;
            } else
                cout << "Draw" << endl;
