...
void solve() {
    ...
    /*
        concept =>
        for each v[i] , we have to 2 options
        1. either this elements denots the length of subsequent  segment  Lenght[segment...]
        2. v[i] denotes length of previous segment

        Case1:
            if(v[i] = length of next segment){
                move i = i+v[i]+1
            }
        Case2:
            if(v[i] = length of previous segment){
                we must have v[i] elements before i
                and i-v[i]-1 must be good
            }

        a segment is good if it satisfy either of the condition
