...
int rec(int i, int target) {
    ...
    int notPick = rec(i + 1, target);
    int Pick = rec(i + 1, target - v[i]);
    int ans = (notPick % MOD + Pick % MOD) % MOD;
...
void solve() {
    ...
    int targetSum = tsum / 2;
    memset(dp, -1, sizeof(dp));
    // cout << rec(0, targetSum) / 2 << endl;
    int ans = rec(0, targetSum);
    cout << (ans * modInverse(2)) % MOD << endl;
    // we are given no from 1 to N
    // we have to choose a subset such that it's sum is equal to targetSum
    // 1 2 3 4 5 6 7 , targetSum = 14
    //
    // if (sum % 2)
    // {
    //     cout << 0 << endl;
    ...
    //     }
    // }
    // cout << dp[n][sum] * modInverse(2) % MOD << endl;
}
signed main() {
    fastio();
