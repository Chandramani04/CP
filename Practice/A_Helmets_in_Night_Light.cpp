...
void solve() {
    ...
    input(a, n);
    input(b, n);
    vector<pair<int, int>> cw, reach_wise;
    for (int i = 0; i < n; i++) {
        reach_wise.push_back({a[i], b[i]});
        cw.push_back({b[i], a[i]});
    }
    sorted(reach_wise);
    sorted(cw);
    // jitne messangers ki cost < p hai vo calculate krlo
    // and jo bache hain unko p use krke inform krdo
    int ans = p, reach = 0;
    int target = n - 1;
    int i = 0;
    for (i; i < n; i++) {
