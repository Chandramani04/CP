...
void solve() {
    ...
    auto ok = [&](int mid) {
        int extra_task_can_be_done = 0, task_cannot_done_by_worker = 0;
        repo(worker, 1, n) {
            if (mid >= mp[worker])
                extra_task_can_be_done += (mid - mp[worker]) / 2;
            else
                task_cannot_done_by_worker += mp[worker] - mid;
        }
        return task_cannot_done_by_worker <= extra_task_can_be_done;
    };
    int l = 0, r = 2 * m + 1;
    // binary search of 0000000011111 case
    while (r - l > 1) {
        int mid = l + (r - l) / 2;
        if (ok(mid)) {
            r = mid;
