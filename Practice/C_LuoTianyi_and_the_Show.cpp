...
void solve() {
    ...
    if (sz(v) == 0) {
        cout << min(m, max(c1, c2)) << endl;
        return;
    }
    int ans = min(m, max(c1, c2));
    sorted(v);
    v.erase(unique(all(v)), v.end());
    if (c1 == 0) {
        int temp = c2 + sz(v);
        ans = max(ans, min(m, temp));
        cout << ans << endl;
        return;
    }
    if (c2 == 0) {
    ...
    // to use c2 and c1 both , we have to use atleast 1 element of v

    // [c2 c2 c2 c2 v[i] v[i] v[i]]

    /*
                int left_part = min(v[i] - 1, c1);
        c1-=left_part;
        if(c1<0)c1 = 0 ;
        int r = sz(v) - i - 1;  // right elements present in the array after i
        // lefts c1 can be used insteaed of c2
        int right_part = min(m - v[i], r + max(c1,c2));
        ans = max(ans, left_part + right_part + 1);
    */
    ans = max(ans, min(m, c2 + sz(v)));
    ans = max(ans, min(m, c1 + sz(v)));

    rep(i, 0, sz(v)) {
        int left_side = v[i] - 1, right_side = sz(v) - i - 1, right_side_elements = m - v[i], left_side_elements = i;

        int left_use = min(left_side, c1 + i);                      // place v[i] first and then use c1
        int right_use = min(right_side_elements, right_side + c2);  // first place v[i] then  use c2 + right side elements to fill right side array
        ans = max(ans, min(m, left_use + right_use + 1));
        // use c2 to start and ignore c1
        int onlyusec2 = min(left_side, c2);  // use c2 to fill left side
        ans = max(ans, onlyusec2 + right_side + 1);
        if (ans > m) ans = m;

        // use c1 to start and ignore c2
        int onlyusec1 = min(right_side_elements, right_side + c1);  // use c1 to fill right side

        ans = max(ans, onlyusec1 + left_side_elements + 1);

        if (ans > m) ans = m;
        // use complete c1 and some c2

        // use complete c2 and some c1
    }
    cout << ans << endl;
}
signed main() {
