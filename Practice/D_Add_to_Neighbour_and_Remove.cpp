...
void solve() {
    ...
    auto check = [&](int reqArraySize, int reqSubSum) {
        int cnt = 0;
        int s = 0;
        for (int i = 0; i < n; i++) {
            s += v[i];
        ...
        return cnt == reqArraySize;
    };

    int ans = n;

    vi posArraySize = factors(tsum);
    for (auto x : posArraySize) {
        if (tsum % x == 0 and x<=n) {
            // then value of each elements should be
            int val = tsum / x;
            if (check(x, val)) {
                ans = min(ans, n - x);
            }
            // now we have to check if we can create x distinct subarrays with each having sum = val
        }
    }
    cout << ans << endl;
}
