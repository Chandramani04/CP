...
void solve() {
    ...
    cin >> n;
    int rem = 2 * n;
    bitset<32> x(n), a(0), b(0);
    // cout << " x = " << x << endl;
    int ga = 0, gb = 0;
    int i = 31;
    while (x[i] == 0) i--;
    for (i; i >= 0; i--) {
        // if xor[i] =  0 , can give both a and b or don't give
        if (x[i] == 0) {
            int if_given = 2 * (1 << i);
            if (if_given <= rem) {
                a[i] = 1;
            ...
            // can only give if 2 * (1<<i) <=sum
        } else {
            // can give to only 1
            int if_given = (1 << i);
            if (if_given <= rem) {
                // give to a  if (ga==0 or ga>0 and gb>0)
                if (ga == 0 or (ga > 0 and gb > 0)) {
                    a[i] = 1;
                    ga++;
                } else {
                    b[i] = 1;
    ...
    // cout<<a<<" "<<b<<endl;
    // int fa = a.to_ullong(), fb = b.to_ullong();
    // if (rem > 0 or (fa == 0 or fb == 0)) {
    //     cout << -1 << endl;
    //     return;
    // }
    // cout << fa << " " << fb << endl;
