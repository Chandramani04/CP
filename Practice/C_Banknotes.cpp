...
void solve() {
    ...
    moves.pb(0);
    rep(i, 1, n) {
        moves.pb((coin[i] - coin[i - 1]) / coin[i - 1]);
    }
    // we have to find how much minimum amount we can make using k+1 coins
    int done = 0;
    rep(i, 1, sz(moves)) moves[i] += moves[i - 1];
    int it = lb(moves, k + 1);
    if (it == sz(moves)) {
        // use all the coins till n-1 and rem_moves = k+1-moves[it-1];
        int use = coin[n - 1] - 1;
        int rem_moves = k + 1 - moves[it - 1];
        use += rem_moves * coin[n - 1];
        cout << use << endl;
    } else {
        int use = coin[it - 1] - 1;
        int rem_moves = k + 1 - moves[it - 1];
        use += rem_moves * coin[it - 1];
        cout << use << endl;
    }
}
signed main() {
