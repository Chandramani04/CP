...
/*---------------------------Code Begins---------------------------------------*/
char str[1000000];
int rec(int i, string& s, char prev) {
    int n = sz(s);
    if (i == n) return 0;
    if (s[i] == '?') {
        // what was previous character
        if (prev == 'B') {
            int op1 = rec(i + 1, s, 'R'), op2 = 1 + rec(i + 1, s, 'B');
            if (op1 <= op2) {
                str[i] = 'R';
                return op1;
            } else {
            ...
            int op1 = rec(i + 1, s, 'B'), op2 = 1 + rec(i + 1, s, 'R');
            if (op1 <= op2) {
                str[i] = 'B';
                return op1;
            } else {
        ...
        // we have fixed character
        if (s[i] == prev) {
            str[i] = s[i];
            return 1 + rec(i + 1, s, prev);
        } else {
            str[i] = s[i];
            return rec(i + 1, s, s[i]);
        }
    }
}
void solve() {
