...
we only care about face up sides of the cards
*/
int cache[200100][2];
int rec(int i, int pid) {
    if (i == n) {
        return 1;
    }
    if (cache[i][pid] != -1) return cache[i][pid];
    int ans = 0;
    if (pid == 1) {
        // previous card was from b array
        if (b[i - 1] == a[i]) {
            // we have to check wheather we can use b[i] or not , ie newpid = 1
            // we must flip
            if (b[i - 1] == b[i]) {
                // flipping won't help
                return 0;
        ...
        // previous card was from a array
        if (a[i - 1] == a[i]) {
            if (a[i - 1] == b[i]) {
                return 0;
            } else {
...
void solve() {
    ...
    int ans = (rec(1, 0) % mod + rec(1, 1) % mod) % mod;
    cout << ans << endl;
}
signed main() {
    fastio();
