...
bool doIntersect(int a1, int b1, int a2, int b2) {
    if (a1 > b1)
        swap(a1, b1);
    if (a2 > b2)
        swap(a2, b2);
...
void solve() {
    ...
    // if one point in between x and y  and other one is outer
    if ((c >= y and c <= x and d <= y) or (c >= y and c <= x and d >= x) or (d >= y and d <= x and c <= y) or (d >= y and d <= x and c >= x)) {
        cout << "YES" << endl;
    } else
        cout << "NO" << endl;
}
