    ...
    */
    ...
    void solve() {
        ...
        // make K palindromic strings
        // first distribute even cnt equally to each [1....k] strings
        // distribute odd cnt -> if odd cnt >=k we can place 1 odd in each kth string and increase it's size by 1
        // print(mp);
        int even = 0, odd = 0;
        for (auto [x, y] : mp) {
            even += y / 2;
        ...
        int no = odd + (even - de * k);
        int final_seg = de + (no >= k);
        cout<<final_seg<<endl;
        return;
