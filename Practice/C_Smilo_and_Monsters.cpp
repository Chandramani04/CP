...
void solve() {
    /*
        CONCEPT =>
        S = sum of all monsters
        // kill S/2 monsters individually => ans = S/2 , bonusAttack = S/2 { we can use this for 2nd type of atatck }
        // if S is odd => ans = S/2 + 1
        // Now bache hue S/2 monsters to kill krne ke liye hmko 1 mega attack each use krna hoga
        // we will kill the monster with maximum health first so that we can use minimum 2nd type attack
        // each 2nd type attack costs 1
        // now find how many monsters will add up sum just equal to or greater than  S/2
        // add those #monsters to ans
