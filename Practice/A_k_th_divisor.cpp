...
void solve() {
    ...
    */
    // print(div);
    int growing = sz(div);
    int max_pos = 2 * growing;
    if (n == div[growing - 1] * div[growing - 1]) max_pos--;
    if (k > max_pos) {
        print(-1);
        return;
    }
    if (k <= growing) {
        ...
        for (int i = sz(div) - 1; i >= 0; i--) {
            if (n / div[i] != div[i]) v.pb(n / div[i]);
        }
        /*
            int req = k - grwoing  from last
            req = 5 - 3 = 2
            int ans = n/div[growing-req];
        */
        int req = k - growing;
        cout << v[req - 1] << endl;
        return;
