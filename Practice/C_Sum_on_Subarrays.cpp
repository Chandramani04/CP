...
void solve() {
    /*
        Concept =>
        we have to make K subarray +ve and remaining -ve
        let N = 6 , k = 10
            _ _ _ _ _ _
        i = 0 1 2 3 4 5

        for each i , we can create a maximum subarray of length N - i

        Construct the array in such a fashion that all +ve numbers are in the starting and -ve in the end

        if at a current i =>
        k>=n-i {
            we must make this subarray of length n-i +ve and decrease value of k by n - i
        }
        else{
            if k == 0 , then we must make all the remaining elements -1
            else we can make the subarray of length K [i..i+k-1] +ve and remaining -1
            ie. =>
                i i+1 i+2 i+3 i+k-1  => make this K length subarray +ve and further elements/subarrays -ve

                how to make [i...i+k-1] +ve and remaining -ve
                give v[i] = k
                give [i+1 to i+k-1] = -1

                now sum from i to i+k-1 = k + (-1)*(k-1) =  1 => this means this subarray is a positive subarray

                now if we give elements from i+k to n-1 as -2 , this subaray will be -ve
        }
