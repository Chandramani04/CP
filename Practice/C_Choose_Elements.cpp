...
int arr[2][200020];
/*
    x[i] = a[i] or b[i] for i = 1 to n
    abs(x[i]-x[i+1]) <= k for i = 1 to n-1
*/
int cache[200020][2];
int f(int i, int prev) {
    ...
    // check if we can jump from i to i+1 with prev as prev
    int sameLine = 0, diagonalLine = 0;
    if (abs(arr[prev][i] - arr[prev][i + 1]) <= k) {
        sameLine = f(i + 1, prev);
    }
    if (abs(arr[prev][i] - arr[1 - prev][i + 1]) <= k) {
        diagonalLine = f(i + 1, 1 - prev);
    }
    return cache[i][prev] = sameLine | diagonalLine;
}
