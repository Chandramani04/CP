...
void solve() {
    // find mex
    // every ele < mex should occurs more than once
    int n;
    cin >> n;
    input(v, n);
    set<int> st(all(v));
    ...
    // 1st subarray jisme sare elements < mex hai usko find out kro
    vi fs;
    set<int> s1;
    int start = 0;
    for(int i = 0;i<n;i++){
    ...
    // // vi f1 = frequency(r1), f2 = frequency(r2);
    // map<int, int> mp1, mp2;
    // for (auto x : r1) {
    //     mp1[x]++;
    // }
    ...
    // }
    // for (int i = 0; i < mex; i++) {
    //     if (mp1[i] == 0 or mp2[i] == 0) {
    //         cout << -1 << endl;
    //         return;
    //     }
    // }
    
    // 2nd subarray me bhi jo element bache hai unki frequency > 1 honi chahiye
}
signed main() {
    fastio();
