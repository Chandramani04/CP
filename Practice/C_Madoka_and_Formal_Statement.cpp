...
void solve() {
    ...
    // start sbse chhote element se krenge
    // and usko bada krte jayenge
    rep(i, 0, n) {
        int id = mp[v[i]].front();
        mp[v[i]].pop_front();
        // int id ;
        ...
        // next element kitna badha skte hain maximum ==b[id+1] jitna
        int next_max = b[id + 1];
        int cur_max = next_max + 1;
        a[id] = min(b[id], cur_max);
    }
    rep(i, 0, n) {
        if (a[i] != b[i])
            if (ok) ok = false;
