...
void solve() {
    ...
    cin >> n >> k;
    input(v, n);
    // for each v[i] , calculate the minimum sum of the array by changing k adjacent elements to v[i]
    int tsum = sum(v);
    if(k==0){
        cout<<tsum<<endl;
        return;
    ...
    rep(i, 0, n) {
        int curSum = tsum;
        // k time we can change the adjacent elements to v[i]
        for (int goRightSide = 1; goRightSide <= k; goRightSide++) {
            for (int goLeftSide = 1; goLeftSide <= k; goLeftSide++) {
                if ((goRightSide + goLeftSide <= k) and (i + goRightSide < n and i - goLeftSide >= 0)) {
                    int temp = curSum;
                    temp -= v[i + goRightSide];
                    temp -= v[i - goLeftSide];
                    temp += v[i];
                    ans = min(ans, temp);
                }
