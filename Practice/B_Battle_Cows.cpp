...
void solve() {
    ...
    auto fn = [&](vi arr) {
        ...
        // print(arr);
        cnt = count(arr.begin() + 1, arr.end(), cow);
        return cnt;
    };
    int ans = fn(v);
    swap(v[0], v[k]);
    ...
    if (strong != -1 and strong < k) {
        // only winning chance is before strong cow position , after that someone is stronger than our cow
        // so either we replace our cow with the strong cow or we place it at start
        swap(v[k], v[strong]);
        ans = max(ans, fn(v));
        swap(v[k], v[strong]);
    }
