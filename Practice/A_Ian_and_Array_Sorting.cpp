...
void solve() {
    ...
    // make first pair of elements as small as possible
    int minf = -1e18;
    // to make v[0] = minf and v[1] = minf
    int dif = v[0] - minf;
    v[0] -= dif;
    v[1] -= dif;
    int mxinf = 1e18;
    ...
    rep(i, 1, n - 1) {
        // if my current element is smaller than previous element
        if (v[i] < v[i - 1]) {
            dif = v[i - 1] - v[i];
            v[i] += dif;
            v[i + 1] += dif;
        } else if (v[i] > v[i - 1]) {
            // we will decrease the difference
            dif = v[i] - v[i - 1];
            v[i] -= dif;
            v[i + 1] -= dif;
        }
