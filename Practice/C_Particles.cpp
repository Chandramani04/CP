...
void solve() {
    ...
    if (gz == 0) {
        // we will only take maximum elements from the deque
        cout << maxel(v) << endl;
        return;
    } else {
        // remove all negative elements from the deque from both sides
        ...
        // print max of c0 anc c1
        cout << max(max(c0, 0ll), max(c1, 0ll)) << endl;
    }
}
signed main() {
    fastio();
