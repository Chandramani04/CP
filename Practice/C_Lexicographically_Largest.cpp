...
void solve() {
    ...
    reverse(hash);
    vi ans;
    int border_pos = n;  // border_pos indicates if we have a number with position greater than border pos
    // bordre_pos => largest element position border right side
    // then we decrease the number with minus value;
    int minus_val = 0;
    int right_side_del = 0;
    rep(i, 0, sz(hash)) {
        vi occ = hash[i].second;
        sorted(occ);
        int lele = hash[i].first;
        rep(j, 0, sz(occ)) {
            if (occ[j] >= border_pos) {
                if (right_side_del + border_pos == n) {
                    ans.pb(lele);
                } else
                    ans.pb(lele - minus_val);
                // minum_val will be applied

            } else {
                // minus_val won't be applied
                if (right_side_del + border_pos == n) {
                    ans.pb(lele - minus_val);
                } else
                    ans.pb(lele);
                // bordre_range will be changes and decreaseed
                border_pos = occ[j];
                minus_val++;
            }
            right_side_del++;
    ...
    //     rep(j, 0, sz(occ)) {
    //         int first_occ = occ[j];
    //         int final_ele_after_decreasing = larest_ele - minus_val;
    //         if (first_occ > border_pos) {
    //             // current element is at larger side of the border
    //             // what is next largest element in the hash
    //             if (i < sz(hash) - 1) {
    //                 int next_largest = hash[i + 1].first;
    //                 int next_largest_firstocc = hash[i + 1].second[0];
    //                 int final_next = -1e9;
    //                 if (next_largest_firstocc > border_pos) {
    //                     final_next = next_largest - minus_val;
    //                 }
    //                 if (final_ele_after_decreasing < final_next) {
    //                     break;
    //                 } else {
    //                     // pick this element in the B array
    //                     ans.push_back(final_ele_after_decreasing);
    //                 }
    //             }
    //         } else {
    //             // current element is at smaller side of the border
    //             // pick this element in the B array
    //             ans.push_back(larest_ele);
    //             border_pos = first_occ;
    //             minus_val++;
    //         }
    //     }
    // }
