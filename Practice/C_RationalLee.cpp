...
void solve() {
    ...
    input(a, n);
    input(w, k);
    // Give greatest elements to friends with wi=1
    sort(a.begin(), a.end());
    sorted(w);
    ...
    // For the rest sort the elements in non-descending order of ai
    // and sort the friends in non - ascending order of wi  => already sorted both arrays

    /*
    then give first w1−1
    elements to friend 1
    , next w2−1
    elements to friend 2
    and so on, also give k−i+1
    -th greatest element to friend i
    (1≤i≤k)
    */

    // for each w[i] => give smallest w[i]-1 ele to i and greatest ele to i
    int i = 0, j = sz(a) - 1;
    for (int x : w) {
        ans += a[i] + a[j];
        i += x - 1;
