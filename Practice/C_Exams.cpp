...
void solve() {
    ...
    int ans = 0;
    rep(i, 0, n) {
        int exam_no = v[i].first;
        int option_date = v[i].second;
        // for 1st exam he will try to choose minimum date
        if (i == 0) {
            ans = max(ans, min(exam_no, option_date));
        } else {
            // if my optional date is >=ans  , then i will choose that date
            // else i will chose my original exam date
            if (option_date >= ans) {
                ans = option_date;
            } else {
                ans = exam_no;
