...
void solve() {
    ...
    auto check = [&](int k) {
        deque<int> dq = p;
        int move = k;
        for (move = k; move >= 1; move--) {
            while (!(dq.empty()) and dq.back() > move) {
                dq.pop_back();
            }
            if (dq.empty()) return false;
            dq.pop_back();  // alice turn
            if (dq.empty()) return move == 1;
            // bob turn
