...
void solve() {
    ...
    vi temp = v;
    int cnt = 0;
    unordered_map<int, vi, custom_hash> mp;
    vi winner;
    rep(i, 0, n - 1) {
        if (temp[i] == mx or temp[i + 1] == mx) {
            break;
        }
        cnt++;
        temp[i + 1] = max(temp[i], temp[i + 1]);
        winner.pb(temp[i + 1]);
        // mp[i] -> stores jth round in which i won
        // mp[temp[i + 1]].push_back(i + 1);
    }
    // cout << "winners : ";
    // print(winner);
    // cout << "queries :" << endl;
    while (q--) {
        ...
        if (s == mx) {
            ...
            auto f = lower_bound(winner.begin(), winner.begin() + min(cnt, r), s);
            int fid = f - winner.begin();
            auto l = upper_bound(winner.begin(), winner.begin() + min(cnt, r), s);
            int lid = l - winner.begin();
            cout << lid - fid << endl;
            // if (f == winner.end()) {
            //     cout << 0 << endl;
            // } else {
            ...
            // t = { 1,1,2,2,2,2,2,3,4}
            // lb(2) = 2 , ub(2) = 7
            // winner = {x,x,,x,x,x S S S S yyyyyyyyyyy}
        }
    }
}
signed main() {
