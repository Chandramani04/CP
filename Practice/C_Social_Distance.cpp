...
void solve() {
    ...
    vi temp;
    // for (int i = 1; i < v[0] - k; i++) {
    //     temp.push_back(i);
    // }
    cmd.pb(max(0ll, v[0] - k - 1));
    // store.push_back(temp);
    // temp.clear();
    // for (int i = v.back() + k + 1; i <= n; i++) {
    //     temp.push_back(i);
    // }
    cmd.pb(max(0ll, n - v.back() - k));
    // store.push_back(temp);
    // temp.clear();
    for (int i = 1; i < sz(v); i++) {
        // for (int j = v[i - 1] + k + 1; j < v[i] - k; j++) {
        //     temp.push_back(j);
        // }
        // store.push_back(temp);
        // temp.clear();
        cmd.pb(max(0ll, v[i] - v[i - 1] - 2 * k - 1));
    }
    k++;
    for(auto x:cmd){
        cnt += ceil(x, k);
