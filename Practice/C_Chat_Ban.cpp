...
/*---------------------------Code Begins---------------------------------------*/
int upward_binarysearch(int x, int k) {
    int l = 0;
    int r = k;
    while (r - l > 1) {
        int mid = l + (r - l) / 2;
...
int downward_binarysearch(int x, int k) {
    int l = 0, r = k;
    while (r - l > 1) {
        int mid = l + (r - l) / 2;
        // k k-1 k-2 k-3 ....... 1
        int val = ((mid) * (2 * k - mid + 1)) / 2;
...
void solve() {
    ...
    /*
        ...
        sum of n terms of an AP = ((n)*(2*a + (n-1)*d))/2
        // 7 6 5 4
        // 4 *(14+-3) / 2 = 44/2 = 22
    */
    // check in which half the number lies
    // 1 2 3 4 ....... k
    int upward = (k * (k + 1)) / 2;
    // cout << upward << ":";
    // cout<<downward_binarysearch(x,k);
    if (x <= upward) {
        // find n such that 1 2 3 4 ....... n <=x
        // n(n+1)/2 <= x
        // n^2 + n - 2*x <= 0
        // n = (-1 + sqrt(1+8*x))/2
        // int n = (-1 + sqrt(1 + 8 * x)) / 2;
        int n = upward_binarysearch(x, k);
        cout << n;
        // cout << "1st half" << endl;
    } else {
        // cout << "2nd" << endl;
        ...
        x -= upward;
        k--;
        int newupward = (k * (k + 1)) / 2;
        if (x > newupward) {
            cout << k + k + 1 << endl;
            return;
        }
        cout << k + 1 + downward_binarysearch(x, k);
        // cout << " : 2nd half" << endl;
    }
    cout << endl;
}
