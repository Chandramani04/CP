...
void solve() {
    ...
    for (int team_size = 1; team_size <= n; team_size++) {
        int target = ((x + team_size - 1) / team_size);
        // cout << target << " ";
        if (sz(v) == 0 or team_size > sz(v)) break;
        auto it = lower_bound(v.begin(), v.end(), target);
        if (it == v.end()) {
            // can't make any team with team_size members
            continue;
        }
        int id = it - v.begin();
        int pos_ele = sz(v) - id;
        if (pos_ele >= team_size) {
            int del = pos_ele / team_size;
            // cout << team_size << " " << del << endl;
            teams += del;
            for (int j = 0; j < del * team_size; j++) {
                v.pop_back();
            }
        }
    }
