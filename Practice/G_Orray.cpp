...
void solve() {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        c.pb(bestEle);
        s |= bestEle;
        ms.erase(ms.find(bestEle));
    }
    c.insert(c.end(), all(ms));
    print(c);
    // int por = 0;
    // for (int i = 0; i < n; i++) {
    //     por |= c[i];
    ...
    //     bitset<32> current(s);
    //     if (current[i] == 1 or store[i].empty()) {
    //         continue;
    //     }
    //     int x = store[i].back();
    //     c.pb(x);
