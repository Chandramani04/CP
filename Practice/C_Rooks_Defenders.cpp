...
void solve() {
    ...
    map<int, int> row, col;
    ordered_set a, b;
    // https://codeforces.com/contest/1679/submission/242704734
    while (q--) {
        int type;
        cin >> type;
        if (type == 3) {
            int x1, y1, x2, y2;
            cin >> x1 >> y1 >> x2 >> y2;
            int rowmatch = a.order_of_key(x2 + 1) - a.order_of_key(x1);
            int colmatch = b.order_of_key(y2 + 1) - b.order_of_key(y1);
            if (((row[x1] > 0 and row[x2] > 0) and rowmatch == x2 - x1 + 1) or ((col[y1] > 0 and col[y2] > 0) and colmatch == y2 - y1 + 1)) {
                cout << "Yes\n";
            } else {
                cout << "No\n";
            }
