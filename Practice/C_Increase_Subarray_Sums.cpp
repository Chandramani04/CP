...
//     return max((n % 10) * maxdigitprodcut(n / 10),
//                9 * maxdigitprodcut(n / 10 - 1));
// }
...
//     return (a - b + m) % m;
// }
// // FOR unordered_map<int,int,custom_hash> TO AVOID FALTU TLE'S COZ OF
// // ANTIHASHES.
// struct custom_hash {
...
// int modInverse(int n) { return moduloMultiplication(n, MOD - 2); }
...
// /*---------------------------Code
//  * Begins---------------------------------------*/
// vector<vector<int>> all_subarrays(vi &v) {
//     int n = v.size();
...
//     }
//     */
//    //dp[i] = maximum sum of all subarrays of length i
//     vi dp(n + 1, -1e9);
//     dp[0] = 0;
//     for (int i = 0; i < n; i++) {
//         int s = 0;
//         for (int j = i; j < n; j++) {
//             s += v[j];
//             int sumarray_len = j - i + 1;
//             dp[sumarray_len] = max(dp[sumarray_len], s);
//         }
//     }
//     vi ans;
//     for (int k = 0; k <= n; k++) {
//         long long a = 0ll;
//         // we have to add k time val to distinct elements 

//         for (int i = 0; i <= n; i++) {
//             int tt = (dp[i] + (min(k, i) * val));
//             // from 0 to k , how much time we should add val to get maximum sum of subarray
//             if (tt > a) a = tt;
//         }
//         ans.pb(a);
//     }
...
// std::greater<int>()); if (it == piles.end()) { piles.push_back(num); } else {
//             *it = num;
//         }
//     }
...
int rec(int sum, int copy, int n) {
    if (sum >= n) {
        return 0;
    }
    // copy present sum
    // paste previous copied sum
    int paste_previous = 1e9;
    if (sum + copy <= n) {
        paste_previous = 1 + rec(sum + copy, copy, n);
    }
    int copy_present = 1 + rec(sum, sum, n);
    return min(copy_present, paste_previous);
}
