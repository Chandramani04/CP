...
void solve() {
    int n, k;
    cin >> n >> k;
    // sequence of n terms starting from k  : k, k+1 , k+2,k+3 ...... , k+n-1
    ...
    // find how many terms we need such that sum of first x terms is >=middle
    int need = 0;
    int low = 1, high = n;
    while (low <= high) {
    ...
    int leftSum = (need * (2 * k + need - 1)) / 2;
    int rightSum = sum - leftSum;
    int ans = abs(leftSum - rightSum);
    if (need > 0) {
        need--;
        leftSum = (need * (2 * k + need - 1)) / 2;
