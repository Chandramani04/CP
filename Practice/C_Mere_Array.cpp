...
void solve() {
    ...
    vi samegcd;
    vi notsame;
    set<int> store_samegcd_index;
    rep(i, 0, n) {
        if (arr[i] % mi == 0) {
            samegcd.pb(arr[i]);
            store_samegcd_index.insert(i);
        } else
            notsame.pb(arr[i]);
    }
    // print(samegcd);
    ...
    reverse(notsame);
    for (int i = 0; i < n; i++) {
        if (store_samegcd_index.count(i)) {
            ans.pb(samegcd.back());
            samegcd.pop_back();
        } else {
            ans.pb(notsame.back());
    ...
    // minimum element = 2
    // 4 3 6 6 2 9
    // // find elements whose gcd is 2
    // 4 6 6 2
    // // find elemetns whose gcd is not 2
    // 3 9
    // if [arr when gcd not 2 ] is sorted -> ans is yes
    // else no
}
signed main() {
    fastio();
