...
void solve() {
    ...
    dp[0][1] = 0;
    dp[0][0] = 0;
    // dp[i][0] -> last digit is 0   # beautiful substrings ending at index i with last digit 0 
    // dp[i][1] -> last digit is 1   # beautiful substrings ending at index i with last digit 1
    
    int ans = 0;
    for (int i = 1; i <= n; i++) {
        if (s[i - 1] == '?') {
            dp[i][1] = 1LL + dp[i - 1][0]; // change ? to 1  
            dp[i][0] = 1LL + dp[i - 1][1]; // change ? to 0
        } else if (s[i - 1] == '0') {
            dp[i][1] = 0;
            dp[i][0] = 1LL + dp[i - 1][1]; // change to 1
        } else {
            dp[i][1] = 1LL + dp[i - 1][0]; // change to 0
            dp[i][0] = 0;
        }
