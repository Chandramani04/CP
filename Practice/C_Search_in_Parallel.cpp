...
void solve() {
    ...
    cin >> n >> s1 >> s2;
    input(r, n);
    map<int, vi> storeIndex;
    rep(i, 0, n) {
        storeIndex[r[i]].pb(i + 1);
    }
    vi v = r;
    sorted(v);
    reverse(v);
    /*
        Concept =>
        we have to decide which elements to put in list1 and which in list2
        if a list has size s and we want to put a new element in it the time taken to search for the current elements
         =  (new size of list) * (time taken to search for each element ) * (frequency of element i.e how many times we are searching for this element)

        so we will sort the elements in decreasing order and then we will put the elements in the list which has less cost
        if(costForList1<=costForList2) then we will put the element in list1 else in list2
    */

    int l = 1, j = 1;
    vi list1, list2;
    for (int ele : v) {
        int costForList1 = l * s1 * ele;
        int costForList2 = j * s2 * ele;
        if (costForList1 <= costForList2) {
            list1.push_back(storeIndex[ele].back());
            storeIndex[ele].pop_back();
            l++;
        } else {
            list2.push_back(storeIndex[ele].back());
            storeIndex[ele].pop_back();
            j++;
        }
    }
