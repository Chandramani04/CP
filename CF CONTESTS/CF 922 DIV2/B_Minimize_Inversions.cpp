...
void sort_using_swap(vector<pair<int, int>>& v) {
    int n = sz(v);
    map<int, int> mp;
    ...
    //     if (ele != i + 1) {
    //         int actual = i;
    //         int actual_id = mp[i + 1];
    //         cout << "swapping " << actual << " " << actual_id << endl;
    //         swap(v[actual], v[actual_id]);
    //     }
    // }
}
void solve() {
    ...
    int ans1 = mergeSort(ta, n);
    sorted(b);
    int tb[n];
    rep(i, 0, n) {
        tb[i] = b[i].second;
    }
    int ans2 = mergeSort(tb, n);
    if (ans1 <= ans2) {
        // cout << ans1 << endl;
        for (auto x : a) {
            cout << x.first << " ";
