...
void solve() {
    ...
    vi rightcost(n + 1, 0), leftcost(n + 1, 0);
    repo(i, 1, n - 1) {
        if (nearest[i] == 1) {
            rightcost[i + 1] = 1 + rightcost[i];
        } else {
            rightcost[i + 1] = rightcost[i] + abs(arr[i] - arr[i + 1]);
        }
    }

    for (int i = n; i > 1; i--) {
        if (nearest[i] == -1) {
            leftcost[i - 1] = 1 + leftcost[i];
        } else {
            leftcost[i - 1] = leftcost[i] + abs(arr[i] - arr[i - 1]);
        }
    }
    int m;
    cin >> m;
    while (m--) {
        ...
        cin >> l >> r;
        if (l < r) {
            cout << rightcost[r] - rightcost[l] << endl;
        } else {
            cout << leftcost[r] - leftcost[l] << endl;
        }
    }
}
signed main() {
