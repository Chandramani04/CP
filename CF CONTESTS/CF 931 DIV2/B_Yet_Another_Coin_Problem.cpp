...
int rec(int target) {
    ...
    int temp = 1e9;
    for (auto x : arr) {
        temp = min(temp, rec(target - x) + 1);
    }
    return dp[target] = temp;
}
// int countWaysToMakeChange(int T) {
//     int n = arr.size();
...
void solve() {
    int n;
    cin >> n;
    // find minimum number of coins to make n
    // int ans = n / 15;
    // n = n % 15;
    // vi req = {0, 1, 2, 1, 2, 3, 1, 2, 3, 2, 1, 2, 2, 2, 3};
    // if (n > 0) {
    //     ans += req[n];
    // }
    //     cout << ans << endl;
