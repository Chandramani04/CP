...
void solve() {
    ...
    vector<vector<int>> cnt0(row + 1, vector<int>(col + 1, 0));
    repo(i, 1, row) {
        repo(j, 1, col) {
            if (arr[i][j] == 'o') {
                cnt0[i][j] = 1;
    ...
    vector<vector<int>> cntx(row + 1, vector<int>(col + 1, 0));
    repo(i, 1, row) {
        repo(j, 1, col) {
            if (arr[i][j] == 'x') {
                cntx[i][j] = 1;
    ...
    vector<vector<int>> cnt0_rowprefix(row + 1, vector<int>(col + 1, 0));
    repo(i, 1, row) {
        repo(j, 1, col) {
            cnt0_rowprefix[i][j] = cnt0_rowprefix[i][j - 1] + cnt0[i][j];
        }
    }

    vector<vector<int>> cnt0_colprefix(row + 1, vector<int>(col + 1, 0));
    repo(j, 1, col) {
        repo(i, 1, row) {
            cnt0_colprefix[i][j] = cnt0_colprefix[i - 1][j] + cnt0[i][j];
        }
    }

    vector<vector<int>> cntx_rowprefix(row + 1, vector<int>(col + 1, 0));
    repo(i, 1, row) {
        repo(j, 1, col) {
            cntx_rowprefix[i][j] = cntx_rowprefix[i][j - 1] + cntx[i][j];
        }
    }

    vector<vector<int>> cntx_colprefix(row + 1, vector<int>(col + 1, 0));
    repo(j, 1, col) {
        repo(i, 1, row) {
            cntx_colprefix[i][j] = cntx_colprefix[i - 1][j] + cntx[i][j];
        }
    }

    // repo(i, 1, row) {
    //     repo(j, 1, col) {
    //         cout << cntx_rowprefix[i][j] << " ";
    //     }
    //     cout << endl;
    // }

    // check if there is a continous row or col of size k in cntx
    vi ans;
    repo(i, 1, row) {
        repo(j, 1, col) {
            if (j - k >= 0 and cntx_rowprefix[i][j] - cntx_rowprefix[i][j - k] == 0) {
                // here answer can exist
                // count number of 0's in this k size block
                int cnt = cnt0_rowprefix[i][j] - cnt0_rowprefix[i][j - k];
                ans.pb(k - cnt);
                // cout << i << " " << j << endl;
            }
            if (i - k >= 0 and cntx_colprefix[i][j] - cntx_colprefix[i - k][j] == 0) {
                // cout << i << " " << j << endl;
                int cnt = cnt0_colprefix[i][j] - cnt0_colprefix[i - k][j];
                ans.pb(k - cnt);
            }
        }
    }
