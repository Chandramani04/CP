...
/*---------------------------Code Begins---------------------------------------*/
void solve() {
    int health, gun, laser, reduced_laser, k;
    cin >> health >> gun >> laser >> reduced_laser >> k;
    int ans1 = (health + gun - 1) / gun;
    int before_reducing = k * laser;
    int ans2 = k;
    if (before_reducing >= health) {
        ans2 = (health + laser - 1) / laser;
        cout << min(ans1, ans2) << endl;
        return;
    }
    int dif = health - before_reducing;
    int after = (dif + reduced_laser - 1) / reduced_laser;
    ans2 += after;
    cout << min(ans1, ans2) << endl;
}
signed main() {
